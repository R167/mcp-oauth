...
describe("JWTManager", () => {
	...
	describe("verifyAccessToken", () => {
		it("should verify a valid access token", async () => {
			...
			expect(jwt.verify).toHaveBeenCalledWith("valid.jwt.token", mockEnv.JWT_PUBLIC_KEY, { algorithm: "RS256" });
			expect(result).toEqual(mockPayload);
		});

		it("should throw JWTError for invalid token format", async () => {
			await expect(jwtManager.verifyAccessToken("invalid-token-format")).rejects.toThrow(JWTError);
		});
		...
		it("should throw JWTError for invalid signature", async () => {
			...
			await expect(jwtManager.verifyAccessToken(token)).rejects.toThrow(JWTError);
		});
	...
	describe("verifyRefreshToken", () => {
		it("should verify a valid refresh token", async () => {
			...
			expect(jwt.verify).toHaveBeenCalledWith("decrypted-jwt-token", mockEnv.JWT_PUBLIC_KEY, { algorithm: "RS256" });
			expect(result).toEqual(mockPayload);
		});

		it("should throw JWTError for invalid refresh token format", async () => {
			await expect(jwtManager.verifyRefreshToken("invalid-token-format")).rejects.toThrow(JWTError);
		});
		...
		it("should throw JWTError when decryption fails", async () => {
			...
			await expect(jwtManager.verifyRefreshToken(token)).rejects.toThrow(JWTError);
		});
	...
	describe("extractTokenFromBearer", () => {
		...
		it("should throw JWTError for missing authorization header", async () => {
			await expect(jwtManager.extractTokenFromBearer(null)).rejects.toThrow(JWTError);
		});

		it("should throw JWTError for invalid authorization header format", async () => {
			await expect(jwtManager.extractTokenFromBearer("InvalidFormat test-token")).rejects.toThrow(JWTError);
		});

		it("should throw JWTError for malformed Bearer header", async () => {
			await expect(jwtManager.extractTokenFromBearer("Bearer")).rejects.toThrow(JWTError);
		});
	});
