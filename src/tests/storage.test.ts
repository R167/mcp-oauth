...
describe("StorageManager", () => {
	...
	describe("storeAuthorizationCode", () => {
		it("should store authorization code with correct TTL", async () => {
			...
			expect(mockKV.put).toHaveBeenCalledWith(KV_KEYS.AUTH_CODE(authCode.code), JSON.stringify(authCode), {
		...
		it("should throw StorageError when KV operation fails", async () => {
			...
			await expect(storage.storeAuthorizationCode(authCode)).rejects.toThrow(StorageError);
		});
	});
	...
	describe("getAuthorizationCode", () => {
		...
		it("should throw StorageError when KV get fails", async () => {
			...
			await expect(storage.getAuthorizationCode("test-code")).rejects.toThrow(StorageError);
		});
	...
	describe("deleteAuthorizationCode", () => {
		...
		it("should throw StorageError when delete fails", async () => {
			...
			await expect(storage.deleteAuthorizationCode("test-code")).rejects.toThrow(StorageError);
		});
	});
	...
	describe("storeRefreshToken", () => {
		...
		it("should throw StorageError when store fails", async () => {
			...
			await expect(storage.storeRefreshToken("token-123", "user-456", "scope")).rejects.toThrow(StorageError);
		});
	});
	...
	describe("getRefreshTokenData", () => {
		...
		it("should throw StorageError when get fails", async () => {
			...
			await expect(storage.getRefreshTokenData("token-123")).rejects.toThrow(StorageError);
		});
	});
	...
	describe("deleteRefreshToken", () => {
		...
		it("should throw StorageError when delete fails", async () => {
			...
			await expect(storage.deleteRefreshToken("token-123")).rejects.toThrow(StorageError);
		});
	});
	...
	describe("storeUserSession", () => {
		...
		it("should throw StorageError when store fails", async () => {
			...
			await expect(storage.storeUserSession("session-123", {})).rejects.toThrow(StorageError);
		});
	});
	...
	describe("getUserSession", () => {
		...
		it("should throw StorageError when get fails", async () => {
			...
			await expect(storage.getUserSession("session-123")).rejects.toThrow(StorageError);
		});
	});
	...
	describe("deleteUserSession", () => {
		...
		it("should throw StorageError when delete fails", async () => {
			...
			await expect(storage.deleteUserSession("session-123")).rejects.toThrow(StorageError);
		});
	});

	describe("listRefreshTokensForUser", () => {
		it("should list refresh tokens for a specific user", async () => {
			const tokens = [{ name: "refresh_token:token1" }, { name: "refresh_token:token2" }, { name: "refresh_token:token3" }];
		...
		it("should handle tokens with null values", async () => {
			const tokens = [{ name: "refresh_token:token1" }, { name: "refresh_token:token2" }];
			mockKV.list.mockResolvedValue({ keys: tokens });
			mockKV.get.mockResolvedValueOnce(JSON.stringify({ user_id: "user123" })).mockResolvedValueOnce(null);
		...
		it("should throw StorageError when list fails", async () => {
			...
			await expect(storage.listRefreshTokensForUser("user123")).rejects.toThrow(StorageError);
		});
	});
	...
	describe("revokeAllUserTokens", () => {
		...
		it("should throw StorageError when revocation fails", async () => {
			...
			await expect(storage.revokeAllUserTokens("user123")).rejects.toThrow(StorageError);
		});
	});
