...
describe("ScopeValidator", () => {
	...
	describe("validateScopeFormat", () => {
		...
		it("should reject multiple MCP scopes", () => {
			const result = validator.validateScopeFormat(["mcp:example.com:filesystem", "mcp:example.com:database"]);
	...
	describe("validateDomainMatchesRedirect", () => {
		it("should accept matching domains", () => {
			const result = validator.validateDomainMatchesRedirect("mcp:example.com:filesystem", "https://example.com/callback");
			expect(result.valid).toBe(true);
		});

		it("should reject mismatched domains", () => {
			const result = validator.validateDomainMatchesRedirect("mcp:example.com:filesystem", "https://different.com/callback");
			expect(result.valid).toBe(false);
